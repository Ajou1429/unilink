...
- [x] 폴더 선택 UI: [supabase/functions/drive-folders](../supabase/functions/drive-folders/index.ts) — 사용자가 링크를 붙여넣지 않고 Drive 폴더를 선택
...
5. `supabase functions deploy google-auth drive-watch drive-webhook drive-sync drive-folders drive-profile drive-disconnect drive-renew-channels`
